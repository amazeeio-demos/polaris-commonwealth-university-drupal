...
docker run --rm --net polaris-commonwealth-university-drupal-default amazeeio/dockerize dockerize -wait tcp://mariadb:3306 -timeout 1m
```
...
docker ps --filter label=com.docker.compose.project=polaris-commonwealth-university-drupal | grep Up | grep polaris-commonwealth-university-drupal-nginx-1
docker ps --filter label=com.docker.compose.project=polaris-commonwealth-university-drupal | grep Up | grep polaris-commonwealth-university-drupal-mariadb-1
docker ps --filter label=com.docker.compose.project=polaris-commonwealth-university-drupal | grep Up | grep polaris-commonwealth-university-drupal-php-1
docker ps --filter label=com.docker.compose.project=polaris-commonwealth-university-drupal | grep Up | grep polaris-commonwealth-university-drupal-cli-1

# Should ssh against the cli container by default
docker compose exec -T cli bash -c "env | grep LAGOON=" | grep cli-drupal

# Should have the correct environment set
docker compose exec -T cli bash -c "env" | grep LAGOON_ROUTE | grep polaris-commonwealth-university-drupal.docker.amazee.io
